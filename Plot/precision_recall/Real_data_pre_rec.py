# -*- coding: utf-8 -*-
"""
Created on Tue Jul  2 16:47:27 2024
...
data = pd.read_csv("D:/My_WorkSpace/CNV_program/徐丹同-毕业生材料/1.毕业论文研究资料/1.2 毕业论文的数据、图表电子文档/1.2.1 数据/CNV_F1score-data/human.realDATA.alltype.pre-rec_average.csv")
# 将百分比转化成浮点数
data['positiveRATE'] = data['positiveRATE'].str.rstrip('%').astype('float') / 100
# data = data[data['mapper'] != 'bwa']
data.sort_values(by='mapper', inplace=True)
...
for i, mapper in enumerate(mappers):
    mapper_data_1 = del_data[del_data['mapper'] == mapper]
    row = 0
    col = i
    ax = axes[col]
...
fig.suptitle("Human_realDATA", fontsize=16, fontweight='bold', color='black')

plt.subplots_adjust(hspace=0.5, wspace=0.5)

plt.tight_layout()

plt.savefig('D:/My_WorkSpace/fig/准确率-召回率曲线-修正/真实数据/Human_realDATA_F1_平均-1.tiff', format='tiff', dpi=300)
